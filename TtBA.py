...
class Attacker():
    def __init__(self, args, model, im_orig, imgi, attack_method='TtBA',
    ...
    def attack(self):
        ...
        sys.stdout.write(
            f'\rImg{self.Img_i} query{self.query :.0f} \tIter{0 :.0f} \treal_d={torch.norm(current_best_adv_v[1]):.6f}')
        sys.stdout.flush()
        ...
        for i in range(self.Max_iter):
            ...
            sys.stdout.write(
                f'\rImg{self.Img_i} Que{self.query :.0f} \tIter{i + 1:.0f} \tdist={current_best_adv_vl2[1]:.4f}, L={L:.3f} NVr={ratios:.3f}')
            sys.stdout.flush()
            ...
            self.old_best_adv.adv_v = current_best_adv_v
        ...
        print(f' --AVG_L={sum(total_L) / len(total_L):.3f}, AVG_ra={sum(total_ratios) / len(total_ratios):.3f}',
              end="")
        return bridge_bestKs
